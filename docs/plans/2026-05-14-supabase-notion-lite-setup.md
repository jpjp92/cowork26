...
8. 표 추가 버튼으로 표 생성, 행/열 추가/삭제가 되는지 확인
9. 마크다운 표를 붙여넣었을 때 편집 가능한 표로 변환되는지 확인
10. 새로고침 후 내용이 유지되는지 확인
...
- 표 편집은 Tiptap table extension으로 처리한다.
- 마크다운 표 붙여넣기는 Tiptap 표 노드로 변환한다.
- 본문 자동 저장은 디바운스(약 1.5초) 방식이다.
- 저장 상태 표시는 고정 높이 영역을 사용해 작성창 레이아웃이 흔들리지 않게 한다.
- 제목은 blur 시 저장된다.
```
